...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                if opt.eval_teacher:
                    ...
                    # depth_cv = lowest_cost.squeeze().cpu().numpy() # H, W
                    # im_depth_cv = Image.fromarray(colorize(depth_cv, vmin=None, vmax=None))
                    # im_depth_cv.save(f"cv_depth.jpg")
                    # exit(0)
                    output = depth_decoder(output)
        ...
        print("inference fps is ", (t2-t1)/pred_disps.shape[0])
