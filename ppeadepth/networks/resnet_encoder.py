...
def resnet_multiimage_input(num_layers, pretrained=False, num_input_images=1):
    ...
    if pretrained:
        ...
        if num_layers == 18:
            loaded = model_zoo.load_url(ResNet18_Weights.IMAGENET1K_V1.url)
        else:
            loaded = model_zoo.load_url(ResNet50_Weights.IMAGENET1K_V1.url)
...
class ResnetEncoderMatching(nn.Module):
    ...
    def compute_depth_bins(self, min_depth_bin, max_depth_bin):
        ...
        if self.depth_binning == 'inverse':
            ...
            base = np.log(min_depth_bin)
            it = np.log(max_depth_bin / min_depth_bin)
            
            exp_base = [base + it * i / self.num_depth_bins for i in range(self.num_depth_bins)]
            self.depth_bins = np.exp(exp_base)
        else:
    ...
    def match_features(self, current_feats, lookup_feats, relative_poses, K, invK):
        ...
        for batch_idx in range(len(current_feats)):
            ...
            _lookup_feats = lookup_feats[batch_idx:batch_idx + 1] # 1, 1, 64, H/4, W/4
            _lookup_poses = relative_poses[batch_idx:batch_idx + 1]

            _K = K[batch_idx:batch_idx + 1]
            _invK = invK[batch_idx:batch_idx + 1]
            world_points = self.backprojector(self.warp_depths, _invK) # .shape= num_bins, 4, HW/16; 4 is homogeneous coordinate
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                pix_locs = self.projector(world_points, _K, lookup_pose) # num_bins, h/4, w/4, 2
                warped = F.grid_sample(lookup_feat, pix_locs, padding_mode='zeros', mode='bilinear',
                                       align_corners=True) # num_bins, 64, H/4, W/4
                ...
                edge_mask = edge_mask * current_mask # edge_mask.shape = num_bins, H/4, W/4
                
                # current_feats.shape =  B, 64, H/4, W/4

                diffs = torch.abs(warped - current_feats[batch_idx:batch_idx + 1]).mean(
                    1) * edge_mask # diffs.shape = num_bins, H/4, W/4
            ...
            cost_volume = cost_volume / (counts + 1e-7) # shape = num_bins, H/4, W/4
        ...
        batch_cost_volume = torch.stack(batch_cost_volume, 0) # B, num_bins, H/4, W/4
    ...
    def forward(self, current_image, lookup_images, poses, K, invK,
        ...
        self.projector = Project3D(batch_size=self.num_depth_bins,
                                   height=self.matching_height,
                                   width=self.matching_width)
        self.backprojector.to(current_image.device)
        self.projector.to(current_image.device)
        
        # current_image.shape = B, 3, H, W
        # lookup_images.shape = B, 1, 3, H, W
        # poses.shape = B, 1, 4, 4
        # K.shape = invK.shape = B, 4, 4 
        ...
        # print("hhhh cost_volume ", cost_volume.shape)
        cost_volume *= confidence_mask.unsqueeze(1)
        # print("hhhh cost_volume*mask  ", cost_volume.shape)
        
        post_matching_feats = self.reduce_conv(torch.cat([self.features[-1], cost_volume], 1))
        # post_matching_feats.shape = B, 64, H/4, W/4
        ...
        # features: dim=64, 128, 256, 512, size=1/2, 1/4, 1/8, 1/16
        # lowest_cost.shape = confidence_mask.shape = B, H/4, W/4   
...
class ResnetEncoder(nn.Module):
    ...
    def forward(self, input_image):
        ...
        # print("Resnet ", self.features[-1].shape)
...
class ResnetEncoderDYJ(nn.Module):
    """Pytorch module for a resnet encoder
    """

    def __init__(self, num_layers, pretrained, num_input_images=1, **kwargs):
        super(ResnetEncoderDYJ, self).__init__()
        ...
        # self.linear = nn.Linear(num_input_images, num_input_images-1)
        ...
        self.convs[("pose", 2)] = nn.Conv2d(256, 12, 1)

        self.relu = nn.ReLU()
        self.net = nn.ModuleList(list(self.convs.values()))

    def forward(self, input_image):
        # input_image.shape = B, 3, 3, H, W
        input_image = rearrange(input_image, 'b c t h w -> b (c t) h w')
        ...
        dec_input = self.features[-1] # B, 512, H/32, W/32
        
        out = self.relu(self.convs["squeeze"](dec_input))
        for i in range(3):
        ...
        out = 0.01 * out.view(-1, self.num_input_images-1, 6)
        ...
        # expected : axisangle.shape = (B, 6) translation.shape = (B, 6)
        # output expected (B, 3), (B, 3) (B, 3) (B, 3)

        return axisangle[:, 0], translation[:, 0], axisangle[:, 1], translation[:, 1]                        



if __name__ == "__main__":
    # model = ResnetEncoder(18, True)
    # model = ResnetEncoderMatching(18, True, 192, 640, adaptive_bins=True)
    # model.eval()
    # x = torch.rand(2, 3, 192, 640)
    # look_up_frames = torch.rand(2, 2, 3, 192, 640)
    x = torch.rand(2, 3, 3, 192, 640)
    model = ResnetEncoderDYJ(18, True, 3)

    y = model(x)
    for item in y:
        print(item.shape)

    # model = ResnetEncoderMatching(18, )
